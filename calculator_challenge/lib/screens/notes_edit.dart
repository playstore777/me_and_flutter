...
class _NotesEdit extends State<NotesEdit> {
  ...
  void handleNoteDelete() async {
    print('_insertNote: ${_providerData.getSecretText()}, ${widget.args}');
  ...
  Future<void> _insertNote(Note note) async {
    print('_insertNote: ${_providerData.getSecretText()}');
    NotesDatabase notesDb =
        NotesDatabase(_providerData.getSecretText() ?? 'dummy');
    await notesDb.initDatabase();
  ...
  Widget build(BuildContext context) {
    _providerData = Provider.of<Calculator>(widget.args[2], listen: false);
    return WillPopScope(
      onWillPop: () async {
        handleBackButton();
        return true;
      ...
      child: Scaffold(
        backgroundColor: Color(NoteColors[this.noteColor]?['l'] as int),
        appBar: AppBar(
          backgroundColor: Color(NoteColors[this.noteColor]?['b'] as int),
            ...
            return GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: Color(entry.value['b'] as int),
                ),
              ),
            );
          }).toList(),
    ...
    return PopupMenuButton(
      ...
      onSelected: (value) {
        onSelectPopupmenuItem(
            parentContext, (popupMenuButtonItems[value]?['name']).toString());
      },
    );
  }
}
